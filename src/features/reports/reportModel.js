...
export const normalizeReport = (report = {}, index = 0) => {
  ...
  return {
    ...
    allowedEmails: [...new Set((Array.isArray(report.allowedEmails) ? report.allowedEmails : [report.allowedEmails]).flatMap(v => String(v || "").split(/[;,\n]/)).map(v => String(v || "").trim().toLowerCase()).filter(Boolean))],
    allowedDomains: [...new Set((Array.isArray(report.allowedDomains) ? report.allowedDomains : [report.allowedDomains]).flatMap(v => String(v || "").split(/[;,\n]/)).map(v => String(v || "").trim().toLowerCase().replace(/^@/, "")).filter(Boolean))],
