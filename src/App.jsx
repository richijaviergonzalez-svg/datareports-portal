      ...
      <div style={{ fontFamily: "'Outfit', system-ui", minHeight: "100vh", background: theme.bg }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 28px", background: theme.bgCard, borderBottom: `1px solid ${theme.border}` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            }} style={{ padding: "5px 14px", borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.bgSurface, cursor: "pointer", fontSize: 11, color: theme.textSecondary, display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="14" height="14" viewBox="0 0 16 16" style={{ color: theme.textSecondary }}><path d="M2 5V3a1 1 0 0 1 1-1h2m6 0h2a1 1 0 0 1 1 1v2m0 6v2a1 1 0 0 1-1 1h-2m-6 0H3a1 1 0 0 1-1-1v-2" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinecap="round"/></svg>
              Pantalla completa
            </button>
    ...
    <div style={{ fontFamily: "'Outfit', system-ui", minHeight: "100vh", background: theme.bg, transition: "background .3s" }}>
      ...
      <div style={{ padding: "24px 28px", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
          ...
          {/* Add report card */}
          {/* Add report card — solo para admins */}
          {isAdmin(user.email) && (
    ...
    if (this.state.hasError) {
      return (
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit', system-ui", background: "#F7F8FA" }}>
          ...
          <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 20, maxWidth: 400, textAlign: "center" }}>
            Ocurrió un error inesperado en el portal. Esto no debería pasar — por favor intentá recargar la página.
          </p>
          <p style={{ fontSize: 11, color: "#9CA3AF", marginBottom: 20, fontFamily: "'JetBrains Mono', monospace", maxWidth: 500, textAlign: "center", wordBreak: "break-all" }}>
            {this.state.error?.message || "Error desconocido"}
          </p>
          <button onClick={() => window.location.reload()} style={{
            padding: "10px 28px", borderRadius: 12, border: "none", background: "#0D9488",
...
function SkeletonLoader() {
  return (
    <div style={{ minHeight: "100vh", background: "#F7F8FA", fontFamily: "'Outfit', system-ui" }}>
      <style>{`@keyframes shimmer { 0% { background-position: -400px 0; } 100% { background-position: 400px 0; } } .skeleton { background: linear-gradient(90deg, #E5E7EB 25%, #F3F4F6 50%, #E5E7EB 75%); background-size: 400px 100%; animation: shimmer 1.5s infinite ease-in-out; border-radius: 8px; }`}</style>
      <div style={{ padding: "12px 28px", background: "white", borderBottom: "1px solid #E2E5EA", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div className="skeleton" style={{ width: 140, height: 28 }}/>
        <div style={{ display: "flex", gap: 8 }}>
          <div className="skeleton" style={{ width: 180, height: 36, borderRadius: 12 }}/>
          <div className="skeleton" style={{ width: 36, height: 36, borderRadius: 12 }}/>
          <div className="skeleton" style={{ width: 100, height: 36, borderRadius: 14 }}/>
        </div>
      </div>
      <div style={{ padding: "24px 28px", maxWidth: 1280, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <div className="skeleton" style={{ width: 140, height: 70, borderRadius: 18 }}/>
            <div className="skeleton" style={{ width: 140, height: 70, borderRadius: 18 }}/>
            <div className="skeleton" style={{ width: 140, height: 70, borderRadius: 18 }}/>
          </div>
        </div>
        <div style={{ display: "flex", gap: 6, marginBottom: 20 }}>
          {[80, 100, 70, 90].map((w, i) => <div key={i} className="skeleton" style={{ width: w, height: 32, borderRadius: 20 }}/>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
          {[1,2,3].map(i => <div key={i} className="skeleton" style={{ height: 240, borderRadius: 22 }}/>)}
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    // Verificar si hay sesión activa de MSAL
    (async () => {
      try {
        const instance = await getMsalInstance();
        const accounts = instance.getAllAccounts();
