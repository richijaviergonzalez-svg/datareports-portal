...
function Dashboard({ user, onLogout }) {
  ...
  const saveAll = useCallback((r, f, rv) => {
    try { localStorage.setItem("datareports-config", JSON.stringify({ reports: r, favorites: f, recentViews: rv })); } catch (e) {}
  }, []);

  const saveReports = (newReports) => { setReports(newReports); saveAll(newReports, favorites, recentViews); };

  useEffect(() => { if (loaded) saveAll(reports, favorites, recentViews); }, [favorites, loaded]);
  ...
  const openReport = (report) => {
    ...
    saveAll(reports, favorites, newRecent);
  };
