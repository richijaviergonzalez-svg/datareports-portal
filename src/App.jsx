...
function Dashboard({ user, onLogout }) {
  ...
  const fetchSharedReports = useCallback(async () => {
    try {
      ...
      const currentIsAdmin = isAdmin(currentUserEmail);

      const params = new URLSearchParams({
        email: currentUserEmail,
        admin: currentIsAdmin ? "true" : "false",
      });

      const res = await fetch(`/.netlify/functions/bi-reports?${params.toString()}`, {
        method: "GET",
        headers: {
          "Accept": "application/json",
          "x-user-email": currentUserEmail,
          "x-user-admin": currentIsAdmin ? "true" : "false",
        },
      });
  ...
  const pushSharedReports = async (newReports) => {
    ...
    try {
      ...
      const res = await fetch("/.netlify/functions/bi-reports", {
        ...
        body: JSON.stringify({ reports: cleanReports, user: { name: user?.name, email: currentUserEmail } }),
      });
      if (!res.ok) {
      ...
      try {
        ...
        const confirm = await fetch(`/.netlify/functions/bi-reports?${confirmParams.toString()}`, {
          method: "GET",
          headers: {
            "Accept": "application/json",
            "x-user-email": currentUserEmail,
