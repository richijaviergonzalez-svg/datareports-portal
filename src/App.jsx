...
const buildActionSummary = (type, report, details = "") => {
...
.report-immersive-fullscreen {
  width: 100vw !important;
  height: 100dvh !important;
  max-width: 100vw !important;
  max-height: 100dvh !important;
}
.report-immersive-fullscreen iframe,
.report-immersive-fullscreen > div,
.report-immersive-fullscreen [id^="pbi-container-"] {
  width: 100vw !important;
  height: 100dvh !important;
}
...
function Dashboard({ user, onLogout }) {
  ...
  const saveReports = (newReports) => {
    ...
    if (added.length > 0) {
      ...
      saveAll(newReports, favorites, recentViews, newNotifs, requests);
    } else {
      saveAll(newReports, favorites, recentViews, notifications, requests);
    }
  };
  ...
  const renderViewerDetailPanel = () => detailReport && (
    <div style={{ position: "fixed", inset: 0, zIndex: 120, display: "flex", justifyContent: "flex-end" }} onClick={() => setDetailReport(null)}>
          ...
          return (<>
            <div style={{ padding: "24px 28px 20px", borderBottom: `1px solid ${theme.border}`, background: dark ? `linear-gradient(135deg, ${colors.darkBg}, transparent)` : `linear-gradient(135deg, ${colors.bg}, transparent)` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                ...
                <button onClick={() => setDetailReport(null)} style={{ width: 34, height: 34, borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.bgSurface, cursor: "pointer", color: theme.textMuted, fontSize: 16 }}>×</button>
              </div>
            ...
            <div style={{ flex: 1, overflow: "auto", padding: "20px 28px 28px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
                {[{ label: "Responsable", value: "Equipo BI" }, { label: "Acceso", value: "Corporativo" }, { label: "Fuente", value: "Power BI Service" }, { label: "Estado", value: statusConfig[detailReport.status]?.label || "Activo" }].map((item, i) => (
              ...
              {isAdmin(user.email) && (
                <div style={{ background: theme.bgSurface, borderRadius: 14, padding: 16, border: `1px solid ${theme.border}`, marginBottom: 18 }}>
                  ...
                  {[{ l: "Report ID", v: detailReport.id }, { l: "Workspace ID", v: detailReport.groupId || "No configurado" }].map((item, i) => (
            ...
            <div style={{ padding: "16px 28px", borderTop: `1px solid ${theme.border}`, display: "flex", gap: 10 }}>
              {!isMaintenance && <button onClick={() => { setDetailReport(null); openReport(detailReport); }} style={{ flex: 1, padding: "14px", borderRadius: 14, border: "none", background: `linear-gradient(135deg, ${T.teal}, ${T.tealDark})`, color: "white", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Abrir reporte</button>}
              <button onClick={() => setDetailReport(null)} style={{ padding: "14px 22px", borderRadius: 14, border: `1px solid ${theme.border}`, background: theme.bgCard, color: theme.textSecondary, cursor: "pointer" }}>Cerrar</button>
            </div>
          </>);
        })()}
      </div>
  ...
  if (selectedReport) {
    ...
    const resizeCurrentPowerBI = () => {
      const container = document.getElementById(`pbi-container-${selectedReport.id}`);
      if (!container) return;
      try {
        const embedded = powerbiService.get(container);
        if (embedded?.resize) embedded.resize();
      } catch (err) {}
    };

    const fullscreenToggle = async () => {
      const el = document.getElementById("report-embed-container");
      ...
      if (viewerFullscreen) {
        setViewerFullscreen(false);
        setTimeout(resizeCurrentPowerBI, 250);
        return;
      }
      ...
      // En mobile, especialmente iOS/Safari, el Fullscreen API no siempre funciona para divs/iframes.
      // Por eso usamos un modo inmersivo dentro de la app como fallback estable.
      if (isMobileLike) {
        setViewerFullscreen(true);
        setTimeout(resizeCurrentPowerBI, 300);
        return;
      }
    ...
    return (
      <div style={{ fontFamily: "'Outfit', system-ui", height: "100vh", minHeight: 0, overflow: "hidden", background: theme.bg, display: "flex", flexDirection: "column" }}>
        ...
        {/* Header */}
        <div className="report-viewer-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", background: theme.bgCard, borderBottom: `1px solid ${theme.border}`, flexShrink: 0, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <button onClick={() => { setViewerFullscreen(false); setSelectedReport(null); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", border: `1px solid ${theme.border}`, borderRadius: 10, background: theme.bgSurface, cursor: "pointer", fontSize: 12, color: theme.textSecondary, transition: "all .2s" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            <button onClick={() => setDetailReport(selectedReport)} title="Ver detalles" style={{ width: 34, height: 34, borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.bgCard, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s" }}>
        ...
        <div className="report-viewer-toolbar" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 20px", background: theme.bgCard, borderBottom: `1px solid ${theme.border}`, flexShrink: 0, flexWrap: "wrap", gap: 6 }}>
          ...
          <div className="report-toolbar-actions" style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span className="report-toolbar-zoom-label" style={{ fontSize: 10, color: theme.textMuted, marginRight: 4 }}>Zoom:</span>
            ...
            <button onClick={fullscreenToggle} title={viewerFullscreen ? "Salir de pantalla completa" : "Pantalla completa"} style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${theme.border}`, background: viewerFullscreen ? T.teal : theme.bgSurface, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .15s" }}>
              <svg width="13" height="13" viewBox="0 0 16 16" style={{ color: viewerFullscreen ? "white" : theme.textSecondary }}><path d="M2 5V3a1 1 0 0 1 1-1h2m6 0h2a1 1 0 0 1 1 1v2m0 6v2a1 1 0 0 1-1 1h-2m-6 0H3a1 1 0 0 1-1-1v-2" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinecap="round"/></svg>
            </button>
          </div>
        </div>
        )}
        ...
        {/* Report embed area */}
        <div className="report-viewer-body" style={{ flex: 1, minHeight: 0, height: "100%", padding: "12px 16px 16px", animation: "fadeUp .3s ease-out", overflow: "hidden" }}>
          <div
            id="report-embed-container"
            className={`powerbi-embed-shell ${viewerFullscreen ? "report-immersive-fullscreen" : ""}`}
            style={{
              height: "100%",
              minHeight: 0,
              background: theme.bgCard,
              borderRadius: viewerFullscreen ? 0 : 16,
              border: viewerFullscreen ? "none" : `1px solid ${theme.border}`,
              overflow: "hidden",
              display: "flex",
              flexDirection: "column",
              position: viewerFullscreen ? "fixed" : "relative",
              inset: viewerFullscreen ? 0 : "auto",
              zIndex: viewerFullscreen ? 9999 : "auto",
            }}
          >
            {viewerFullscreen && (
              <button
              ...
              >
                <svg width="14" height="14" viewBox="0 0 16 16">
                  <path d="M5.5 2H3a1 1 0 0 0-1 1v2.5M10.5 2H13a1 1 0 0 1 1 1v2.5M5.5 14H3a1 1 0 0 1-1-1v-2.5M10.5 14H13a1 1 0 0 0 1-1v-2.5" stroke="currentColor" strokeWidth="1.4" fill="none" strokeLinecap="round"/>
                </svg>
                Salir
              </button>
            )}
            <div style={{ flex: 1, minHeight: 0, height: "100%", overflow: "hidden" }}>
              {isMaintenance ? (
                <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: dark ? "#0D0F14" : "#F9FAFB", padding: 32 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <button onClick={() => { setViewerFullscreen(false); setSelectedReport(null); }} style={{ padding: "10px 24px", borderRadius: 12, border: `1px solid ${theme.border}`, background: theme.bgCard, color: theme.textSecondary, fontSize: 13, cursor: "pointer" }}>Volver al catálogo</button>
                    <button onClick={() => setDetailReport(selectedReport)} style={{ padding: "10px 24px", borderRadius: 12, border: "none", background: T.teal, color: "white", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>Ver detalles</button>
                  </div>
                </div>
              ) : isDraft ? (
                <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: dark ? "#0D0F14" : "#F9FAFB", padding: 32 }}>
  ...
  return (
    <div style={{ fontFamily: "'Outfit', system-ui", minHeight: "100vh", background: theme.bg, transition: "background .3s" }}>
      ...
      <Sidebar dark={dark} collapsed={sidebarCollapsed} setCollapsed={setSidebarCollapsed} activeView={activeView} setActiveView={setActiveView}
        ...
        reports={reports} favorites={favorites} requests={requests} user={user} onLogout={onLogout} isUserAdmin={isAdmin(user.email)}
      ...
      <div className="main-content-responsive" style={{ marginLeft: sidebarWidth, transition: "margin-left .3s cubic-bezier(.4,0,.2,1)", minHeight: "100vh" }}>
                ...
                <div style={{ position: "absolute", top: 44, right: 0, width: 360, background: theme.bgCard, borderRadius: 18, border: `1px solid ${theme.border}`, boxShadow: `0 16px 48px ${dark ? "rgba(0,0,0,.4)" : "rgba(0,0,0,.12)"}`, zIndex: 50, animation: "scaleIn .2s ease-out", overflow: "hidden" }}>
                  ...
                  <div style={{ maxHeight: 320, overflow: "auto" }}>
                    {notifications.length === 0 ? (
                      ...
                      <div key={n.id} onClick={() => { markNotifRead(n.id); if (n.requestId) { const req = requests.find(rr => rr.id === n.requestId); if (req) setSelectedRequest(req); setActiveView("requests"); } else if (n.reportId) { const r = reports.find(rr => rr.id === n.reportId); if (r) openReport(r); } setShowNotif(false); }}
        ...
        <div style={{ padding: "24px 28px" }}>
          ...
          {activeView === "dashboard" && reports.filter(r => r.status === "live").length > 0 && (
            <div style={{ marginBottom: 24, animation: "fadeUp .5s ease-out .3s both" }}>
              ...
              <div className="reports-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
                {reports.filter(r => r.status === "live").slice(0, 4).map((report, i) => {
                  const colors = categoryColors[report.category] || categoryColors.Comercial;
                  return (
          ...
          {/* ====== REPORT DETAIL PANEL ====== */}
          {detailReport && (
            <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", justifyContent: "flex-end" }} onClick={() => setDetailReport(null)}>
                  ...
                  return (<>
                    ...
                    <div style={{ padding: "24px 28px 20px", borderBottom: `1px solid ${theme.border}`, flexShrink: 0, background: dark ? `linear-gradient(135deg, ${colors.darkBg}, transparent)` : `linear-gradient(135deg, ${colors.bg}, transparent)` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                        ...
                        <button onClick={() => setDetailReport(null)} style={{ width: 34, height: 34, borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.bgSurface, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: theme.textMuted, fontSize: 16, transition: "all .2s" }}>×</button>
                      </div>
                    ...
                    <div style={{ flex: 1, overflow: "auto", padding: "20px 28px 28px" }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                        {[
                          { icon: <svg width="14" height="14" viewBox="0 0 16 16"><circle cx="9" cy="7" r="3" stroke="currentColor" strokeWidth="1.3" fill="none"/><path d="M3 19c0-3 3-5 6-5s6 2 6 5" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinecap="round"/></svg>, label: "Responsable", value: "Equipo BI" },
                          { icon: <svg width="14" height="14" viewBox="0 0 16 16"><path d="M8 1.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13z" stroke="currentColor" strokeWidth="1.3" fill="none"/><path d="M5.5 8l2 2 3.5-4" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>, label: "Nivel de acceso", value: "Corporativo" },
                          { icon: <svg width="14" height="14" viewBox="0 0 16 16"><rect x="2" y="2" width="12" height="12" rx="2" stroke="currentColor" strokeWidth="1.3" fill="none"/><path d="M5 6h6M5 8.5h4M5 11h5" stroke="currentColor" strokeWidth="1" strokeLinecap="round"/></svg>, label: "Fuente de datos", value: "Power BI Service" },
                      ...
                      <div style={{ background: theme.bgSurface, borderRadius: 14, padding: 18, marginBottom: 20, border: `1px solid ${theme.border}` }}>
                        ...
                        {[
                          ...
                          <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < 3 ? `1px solid ${theme.border}` : "none" }}>
                            <span style={{ fontSize: 12, color: theme.textMuted }}>{item.l}</span>
                            <span style={{ fontSize: 11, color: T.teal, fontFamily: "'JetBrains Mono', monospace", maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.v}</span>
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={{ padding: "16px 28px", borderTop: `1px solid ${theme.border}`, flexShrink: 0, display: "flex", gap: 10 }}>
                      {isMaintenance ? (
                        ...
                        <button onClick={() => { setDetailReport(null); openReport(detailReport); }} style={{
                      ...
                      <button onClick={() => setDetailReport(null)} style={{
              ...
              return (
                <div key={report.id} onMouseEnter={() => setHoveredCard(report.id)} onMouseLeave={() => setHoveredCard(null)}
                  ...
                  <div style={{ display: "flex", borderTop: `1px solid ${theme.border}` }}>
                    <button onClick={() => isMaintenance ? setDetailReport(report) : openReport(report)}
                      style={{
