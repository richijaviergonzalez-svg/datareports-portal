...
const BRAND_PALETTE = [
  { name: "Lila Pilar", value: "#A37AD6" },
  { name: "Arena", value: "#BEB6AE" },
  { name: "Gris Seda", value: "#AFB2B3" },
  { name: "Celeste Suave", value: "#C3D8E7" },
  { name: "Azul Profundo", value: "#004775" },
  { name: "Lino", value: "#D0CBBC" },
  { name: "Verde Grisáceo", value: "#BAC7C6" },
  { name: "Teal Original", value: "#0D9488" },
];

const withAlpha = (hex, alpha = "18") => `${hex}${alpha}`;
...
const categoryColors = {
  ...
  Tiendas: { bg: "#F0FDFA", accent: "#0D9488", darkBg: "#0D948815", darkText: "#5EEAD4" },
  IH: { bg: "#F5F3FF", accent: "#A37AD6", darkBg: "#A37AD615", darkText: "#C4B5FD" },
  Mayoristas: { bg: "#ECFDF5", accent: "#059669", darkBg: "#05966915", darkText: "#34D399" },
  Comercial: { bg: "#F0FDFA", accent: "#0D9488", darkBg: "#0D948815", darkText: "#5EEAD4" },
  "E-Commerce": { bg: "#EEF2FF", accent: "#4F46E5", darkBg: "#4F46E515", darkText: "#818CF8" },
  Compras: { bg: "#FFFBEB", accent: "#D97706", darkBg: "#D9770615", darkText: "#FBBF24" },
  Producto: { bg: "#FDF2F8", accent: "#DB2777", darkBg: "#DB277715", darkText: "#F472B6" },
  ...
  "Creditos y Cobranzas": { bg: "#EFF6FF", accent: "#004775", darkBg: "#00477520", darkText: "#93C5FD" },
  Deco: { bg: "#F8FAFC", accent: "#64748B", darkBg: "#64748B20", darkText: "#CBD5E1" },
  Contabilidad: { bg: "#F7F3EE", accent: "#BEB6AE", darkBg: "#BEB6AE20", darkText: "#D0CBBC" },
  Prendas: { bg: "#F1F5F9", accent: "#AFB2B3", darkBg: "#AFB2B320", darkText: "#E2E8F0" },
...
const isValidUuid = (value) => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(value || "").trim());
...
const canUserViewReport = (report, user) => {
...
function HealthBadge({ report, dark }) {
  ...
  const statuses = { live: { label: "Conectado al Fabric", color: "#10B981", bg: dark ? "#10B98115" : "#D1FAE5" }, draft: { label: "Sin datos", color: "#F59E0B", bg: dark ? "#F59E0B15" : "#FEF3C7" }, maintenance: { label: "Sin conexión", color: "#EF4444", bg: dark ? "#EF444415" : "#FEE2E2" } };
...
function Dashboard({ user, onLogout }) {
  ...
  const [accentColor, setAccentColor] = useState(() => localStorage.getItem("datareports-accent-color") || "#0D9488");
  ...
  useEffect(() => {
    T.teal = accentColor;
    T.tealLight = accentColor;
    T.tealDark = accentColor;
    T.tealBg = withAlpha(accentColor, "14");
    localStorage.setItem("datareports-accent-color", accentColor);
  }, [accentColor]);
      ...
      <div style={{ fontFamily: "'Outfit', system-ui", height: "100vh", minHeight: 0, overflow: "hidden", background: theme.bg, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", background: theme.bgCard, borderBottom: `1px solid ${theme.border}`, flexShrink: 0, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <button onClick={() => setSelectedReport(null)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", border: `1px solid ${theme.border}`, borderRadius: 10, background: theme.bgSurface, cursor: "pointer", fontSize: 12, color: theme.textSecondary, transition: "all .2s" }}>
            ...
            <div style={{ flex: 1, minHeight: 0, height: "100%", overflow: "hidden" }}>
              {isMaintenance ? (
                <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: dark ? "#0D0F14" : "#F9FAFB", padding: 32 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <button onClick={() => setSelectedReport(null)} style={{ padding: "10px 24px", borderRadius: 12, border: `1px solid ${theme.border}`, background: theme.bgCard, color: theme.textSecondary, fontSize: 13, cursor: "pointer" }}>Volver al catálogo</button>
  ...
  return (
    <div style={{ fontFamily: "'Outfit', system-ui", minHeight: "100vh", background: theme.bg, transition: "background .3s" }}>
      <style>{globalStyles}</style>
      {showAdmin && <AdminPanel reports={reports} onSave={saveReports} onClose={() => setShowAdmin(false)} dark={dark} reportSyncStatus={reportSyncStatus} reportSyncMessage={reportSyncMessage} currentUser={user}/>}
      ...
      <div className="main-content-responsive" style={{ marginLeft: sidebarWidth, transition: "margin-left .3s cubic-bezier(.4,0,.2,1)", minHeight: "100vh" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", background: theme.bgCard, borderBottom: `1px solid ${theme.border}`, position: "sticky", top: 0, zIndex: 20, gap: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ position: "absolute", top: 44, right: 0, width: 360, background: theme.bgCard, borderRadius: 18, border: `1px solid ${theme.border}`, boxShadow: `0 16px 48px ${dark ? "rgba(0,0,0,.4)" : "rgba(0,0,0,.12)"}`, zIndex: 50, animation: "scaleIn .2s ease-out", overflow: "hidden" }}>
                  ...
                  <div style={{ maxHeight: 320, overflow: "auto" }}>
                    {notifications.length === 0 ? (
                      ...
                      <div key={n.id} onClick={() => { markNotifRead(n.id); if (n.requestId) { const req = requests.find(rr => rr.id === n.requestId); if (req) setSelectedRequest(req); setActiveView("requests"); } else if (n.reportId) { const r = userVisibleReports.find(rr => rr.id === n.reportId); if (r) openReport(r); } setShowNotif(false); }}
            ...
            <div style={{ position: "relative" }}>
              <button onClick={() => setShowPalette(!showPalette)} title="Cambiar color de la página" style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.bgCard, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s" }}>
                <svg width="16" height="16" viewBox="0 0 20 20" style={{ color: accentColor }}>
                  <path d="M10 2a8 8 0 0 0 0 16h1.2a1.6 1.6 0 0 0 1.1-2.7l-.2-.2a1.7 1.7 0 0 1 1.2-2.9H15a3 3 0 0 0 3-3C18 5.2 14.4 2 10 2z" stroke="currentColor" strokeWidth="1.4" fill="none" strokeLinejoin="round"/>
                  <circle cx="6.6" cy="8" r="1" fill="currentColor"/><circle cx="9.2" cy="5.8" r="1" fill="currentColor"/><circle cx="12.5" cy="6.3" r="1" fill="currentColor"/><circle cx="13.8" cy="9.3" r="1" fill="currentColor"/>
                </svg>
              </button>
              {showPalette && (
                <div style={{ position: "absolute", top: 44, right: 0, width: 260, padding: 14, borderRadius: 18, background: theme.bgCard, border: `1px solid ${theme.border}`, boxShadow: `0 16px 48px ${dark ? "rgba(0,0,0,.4)" : "rgba(0,0,0,.12)"}`, zIndex: 60, animation: "scaleIn .2s ease-out" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 600, color: theme.text }}>Color de la página</p>
                      <p style={{ fontSize: 10, color: theme.textMuted, marginTop: 2 }}>Paleta Pilar</p>
                    </div>
                    <button onClick={() => setShowPalette(false)} style={{ border: "none", background: "transparent", color: theme.textMuted, cursor: "pointer", fontSize: 16 }}>×</button>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                    {BRAND_PALETTE.map(color => (
                      <button key={color.value} title={`${color.name} ${color.value}`} onClick={() => { setAccentColor(color.value); setShowPalette(false); }} style={{ height: 42, borderRadius: 12, border: `2px solid ${accentColor.toLowerCase() === color.value.toLowerCase() ? theme.text : theme.border}`, background: color.value, cursor: "pointer", boxShadow: accentColor.toLowerCase() === color.value.toLowerCase() ? `0 0 0 3px ${withAlpha(color.value, "22")}` : "none" }} />
                    ))}
                  </div>
                  <button onClick={() => { setAccentColor("#0D9488"); setShowPalette(false); }} style={{ marginTop: 12, width: "100%", padding: "9px 12px", borderRadius: 12, border: `1px solid ${theme.border}`, background: theme.bgSurface, color: theme.textSecondary, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Restaurar color original</button>
                </div>
              )}
            </div>
            {isAdmin(user.email) && (
