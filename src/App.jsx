...
function Dashboard({ user, onLogout }) {
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 28px", background: theme.bgCard, borderBottom: `1px solid ${theme.border}` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={handleExportPDF} disabled={exporting}
              style={{ padding: "6px 14px", borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.bgSurface, cursor: exporting ? "wait" : "pointer", fontSize: 11, color: theme.textSecondary, display: "flex", alignItems: "center", gap: 4, transition: "all .2s", opacity: exporting ? 0.5 : 1 }}>
              {exporting ? (
                <div style={{ width: 14, height: 14, border: `2px solid ${theme.border}`, borderTopColor: T.teal, borderRadius: "50%", animation: "spin .6s linear infinite" }}/>
              ) : (
                <svg width="14" height="14" viewBox="0 0 16 16"><path d="M4 12v2h8v-2m-4-2V2m-3 5l3 3 3-3" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
              )}
  ...
  return (
    <div style={{ fontFamily: "'Outfit', system-ui", minHeight: "100vh", background: theme.bg, transition: "background .3s" }}>
      ...
      {/* Main content area */}
      <div style={{ marginLeft: sidebarWidth, transition: "margin-left .3s cubic-bezier(.4,0,.2,1)", minHeight: "100vh" }}>
        {/* Top bar */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 28px", background: theme.bgCard, borderBottom: `1px solid ${theme.border}`, position: "sticky", top: 0, zIndex: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <h2 style={{ fontSize: 16, fontWeight: 500, color: theme.text }}>
              {activeView === "dashboard" ? "Dashboard" : activeView === "favorites" ? "Favoritos" : "Recientes"}
            </h2>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => setCmdK(true)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: 12, background: theme.bgSurface, border: `1px solid ${theme.border}`, cursor: "pointer", transition: "all .2s" }}>
            ...
            {displayReports.map((report, i) => {
              ...
              return (
                <div key={report.id} onClick={() => openReport(report)} onMouseEnter={() => setHoveredCard(report.id)} onMouseLeave={() => setHoveredCard(null)}
                  ...
                  <p style={{ fontSize: 12, color: theme.textMuted, lineHeight: 1.6, marginBottom: 16, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{report.description}</p>
...
function SkeletonLoader() {
  return (
    <div style={{ minHeight: "100vh", background: "#F7F8FA", fontFamily: "'Outfit', system-ui" }}>
      <style>{`@keyframes shimmer { 0% { background-position: -400px 0; } 100% { background-position: 400px 0; } } .sk { background: linear-gradient(90deg, #E5E7EB 25%, #F3F4F6 50%, #E5E7EB 75%); background-size: 400px 100%; animation: shimmer 1.5s infinite ease-in-out; border-radius: 8px; }`}</style>
      <div style={{ display: "flex" }}>
        <div style={{ width: 260, borderRight: "1px solid #E2E5EA", padding: 20 }}>
