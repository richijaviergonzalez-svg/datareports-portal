    ...
    <div style={{ fontFamily: "'Outfit', system-ui", minHeight: "100vh", background: theme.bg, transition: "background .3s" }}>
      ...
      <div className="main-content-responsive" style={{ marginLeft: sidebarWidth, transition: "margin-left .3s cubic-bezier(.4,0,.2,1)", minHeight: "100vh" }}>
        ...
        <div style={{ padding: "24px 28px" }}>
          ...
          {/* Report Detail Modal */}
          {detailReport && (
            <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setDetailReport(null)}>
              <div onClick={e => e.stopPropagation()} style={{ width: 520, maxHeight: "80vh", background: theme.bgCard, borderRadius: 24, border: `1px solid ${theme.border}`, boxShadow: `0 24px 64px ${dark ? "rgba(0,0,0,.5)" : "rgba(0,0,0,.15)"}`, overflow: "auto", animation: "scaleIn .2s ease-out" }}>
                {(() => { const colors = categoryColors[detailReport.category] || categoryColors.Comercial; return (<>
                  <div style={{ padding: "28px 28px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ width: 56, height: 56, borderRadius: 16, background: dark ? colors.darkBg : colors.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <svg width="24" height="24" viewBox="0 0 22 22" style={{ color: dark ? colors.darkText : colors.accent }}>{iconPaths[detailReport.icon]}</svg>
                    </div>
                    <button onClick={() => setDetailReport(null)} style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${theme.border}`, background: theme.bgSurface, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: theme.textMuted, fontSize: 16 }}>×</button>
                  </div>
                  <div style={{ padding: "16px 28px 28px" }}>
                    <h2 style={{ fontSize: 20, fontWeight: 500, color: theme.text, marginBottom: 6 }}>{detailReport.name}</h2>
                    <p style={{ fontSize: 13, color: theme.textMuted, lineHeight: 1.6, marginBottom: 20 }}>{detailReport.description}</p>
                    <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 11, fontWeight: 500, color: dark ? colors.darkText : colors.accent, background: dark ? colors.darkBg : colors.bg, padding: "5px 14px", borderRadius: 10 }}>{detailReport.category}</span>
                      <StatusBadge status={detailReport.status} dark={dark}/>
                      <HealthBadge report={detailReport} dark={dark}/>
                    </div>
                    <div style={{ background: theme.bgSurface, borderRadius: 14, padding: 18, marginBottom: 20 }}>
                      <p style={{ fontSize: 11, fontWeight: 500, color: theme.textSecondary, marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>Información técnica</p>
                      {[{ l: "Report ID", v: detailReport.id }, { l: "Workspace ID", v: detailReport.groupId || "No configurado" }].map((item, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: i === 0 ? `1px solid ${theme.border}` : "none" }}>
                    ...
                    {detailReport.status === "maintenance" ? (
                      <div style={{ padding: "14px 20px", borderRadius: 14, background: dark ? "#EF444412" : "#FEE2E2", border: `1px solid ${dark ? "#EF444430" : "#FECACA"}`, display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                        <svg width="16" height="16" viewBox="0 0 16 16" style={{ color: "#EF4444", flexShrink: 0 }}><path d="M8 1.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13z" stroke="currentColor" strokeWidth="1.3" fill="none"/><path d="M8 5v3m0 2.5V11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
                        <p style={{ fontSize: 12, color: dark ? "#F87171" : "#991B1B" }}>Este reporte se encuentra en mantenimiento y no está disponible temporalmente.</p>
                      </div>
                    ) : (
                      <button onClick={() => { setDetailReport(null); openReport(detailReport); }} style={{ width: "100%", padding: "14px", borderRadius: 14, border: "none", background: T.teal, color: "white", fontSize: 14, fontWeight: 500, cursor: "pointer", transition: "all .2s" }}>
                        Abrir reporte
                      </button>
                    )}
                  </div>
