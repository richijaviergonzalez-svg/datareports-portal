...
function AdminPanel({ reports, onSave, onClose, dark, reportSyncStatus = "local", reportSyncMessage = "Catálogo local", currentUser }) {
  ...
  const resetForm = () => {
    ...
    setParsed(null);
    setTestResult(null);
    setForm({ id: "", groupId: "", name: "", category: "Comercial", icon: "chart-bar", description: "", status: "live", owner: "Equipo BI", audience: "Corporativo", accessLevel: "Corporativo", dataSource: "Power BI Service", refreshFrequency: "Según dataset", criticality: "media", technicalNotes: "", originalUrl: "", sortOrder: "" });
  };
  ...
  const persistList = async (updated, msg) => {
    setSaving(true);
    try {
      const result = await onSave(normalizeReports(updated));
      const syncedList = normalizeReports(result?.reports || updated);
      setList(syncedList);
      showSuccess(msg || (result?.synced ? "Catálogo sincronizado" : "Cambios guardados localmente"));
      return true;
    } catch (e) {
              ...
              return (
                <div key={report.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderRadius: 16, background: theme.bgCard, border: `1px solid ${editing === report.id ? T.teal : theme.border}`, transition: "all .2s" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: theme.textMuted, marginTop: 3 }}>{report.owner || "Equipo BI"} · {report.refreshFrequency || "Según dataset"} · Criticidad {report.criticality || "media"}</div>
                  </div>
...
function Dashboard({ user, onLogout }) {
  ...
  const fetchSharedReports = useCallback(async () => {
    try {
      ...
      setReportSyncMessage(data.reports?.length ? "Catálogo sincronizado" : "Catálogo listo para sincronizar");
    } catch (e) {
  ...
  const saveReports = async (newReports) => {
    ...
    saveAll(result.reports || cleanReports, favorites, recentViews, updatedNotifs, requests);
    return result;
  };
  ...
  useEffect(() => {
    if (loaded) fetchSharedReports();
  }, [loaded, fetchSharedReports]);
